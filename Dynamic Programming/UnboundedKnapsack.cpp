This File contains UnboundedKnapSack Code and How a recrsive code can be converted into Dynamic Programming

Recursive Function for UnboundedKnapSack:
...
int UnboundedKnapSack(int wt[], int val[], int W, int n) {
	...
	if(t[n][w]!=-1)
		return t[n][w];
	if(wt[n-1]<=W) { 
		return t[n][w] = max(val[n-1]+Knapsack(wt, val, W-wt[n-1], n), Knapsack(wt, val, W, n-1));	
	}
	else{
	...
	for(int i=0; i<=n; i++) {
		for(int j=0; j<=W; j++) { 
			if(n==0 || W==0) 
				t[i][j] = 0;
			if(wt[i-1]<=j) { 
				t[i][j] = max(val[n-1]+t[i][j-wt[i-1]], t[i-1][j]);	
			}
			else{
				t[i][j] = t[i-1][j];
			}
